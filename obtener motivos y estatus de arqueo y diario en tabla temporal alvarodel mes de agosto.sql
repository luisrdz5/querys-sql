...
select r.Numcredito, t.NumCredito, r.FechaPago, t.FechaPago , a.motivo, a.status, a.stsenergia , a.fecfac, r.AnoFact+r.MesFact , convert(varchar,a.FECHAFACTPAGO,112) , convert(varchar,r.FechaFactura,112)
,convert(varchar,a.fechaoperacion,112) , convert(varchar,r.FechaPago,112)
from tmpcredsmotivoago t
left join registrossicomok r on t.IDReferencia = r.ID and r.Numcredito=t.NumCredito and r.FechaPago=t.FechaPago
left join [LINKsac].[Arqueo].[dbo].[Arqueo_2018_07] a on a.rpu=t.RPU and  a.saldo=r.importe and ISDATE(a.FECHAFACTPAGO) = 1 
where convert(varchar,a.FECHAFACTPAGO,112) = convert(varchar,r.FechaFactura,112) 
and a.fecfac = r.AnoFact+r.MesFact
...
join [LINKsac].[Arqueo].[dbo].[Arqueo_2018_07] a on a.rpu=t.RPU and  a.saldo=r.importe
